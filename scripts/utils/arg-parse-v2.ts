...
type ComplexOpt<Opt extends { type: OptTypeSpecifier }> = {
  type: Opt['type'],
  default?: TypeOfOpt<Opt['type']>,
  on: {
    flags: string[],
    type: OptTypeSpecifier,
    use: (v: any, opts: Record<string, any>) => TypeOfOpt<Opt['type']> | undefined,
  }[],
}

type WithComplexOpt<Opts extends Record<string, any>, Key extends string, Opt extends ComplexOpt<any>> =
  Args<Opts & Record<Key, TypeOfOpt<Opt['type']>>>
...
export class Args<Opts extends Record<string, any> = {}> {
  ...
  public boolean<const Key extends string, const Opt extends BasicOpt<boolean>>(key: Key, opt: Opt): WithBasicOpt<Opts, Key, { default: boolean }, boolean> {
    this._options.push({
      ...
      setValue: (_, val, opts) => {
        this._setOpt(key, val.toLowerCase() === 'true', opts);
      },
    });
    return this as any;
  }
  ...
  public complex<const Key extends string, const Opt extends { type: OptTypeSpecifier } & ComplexOpt<Opt>>(key: Key, opt: Opt): WithComplexOpt<Opts, Key, Opt> {
    for (const o of opt.on) {
      this._options.push({
        key,
        flags: o.flags,
        type: o.type,
        default: 'default' in opt ? opt.default : $NoDefault,
        setValue: (_, val, opts) => {
          const used = o.use(val, opts);

          if (used !== undefined) {
            this._setOpt(key, used, opts);
          }
        },
      });
    }
