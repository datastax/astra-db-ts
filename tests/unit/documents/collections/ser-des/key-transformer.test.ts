...
describe('unit.documents.collections.ser-des.key-transformer', () => {
  describe('Camel2SnakeCase', () => {
    const serdes = new CollSerDes({ keyTransformer: new Camel2SnakeCase(), enableBigNumbers: () => 'bigint' });
    ...
    it('should allow for deep transformation', () => {
      const serdes = new CollSerDes({ keyTransformer: new Camel2SnakeCase({ transformNested: () => true }), enableBigNumbers: () => 'bigint' });
    ...
    it('should allow for explicit nested transformation when serializing', () => {
      const serdes = new CollSerDes({ keyTransformer: new Camel2SnakeCase({ transformNested: (ctx) => ctx.path.at(-1) !== '1' }), enableBigNumbers: () => 'bigint' });
    ...
    it('should allow for explicit nested transformation when deserializing', () => {
      const serdes = new CollSerDes({ keyTransformer: new Camel2SnakeCase({ transformNested: (ctx) => ctx.path[0] !== 'camelCaseName3' }), enableBigNumbers: () => 'bigint' });
    ...
    it('should allow for transforming _id', () => {
      const serdes = new CollSerDes({ keyTransformer: new Camel2SnakeCase({ exceptId: false }) });
