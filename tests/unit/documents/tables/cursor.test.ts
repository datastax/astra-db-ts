...
import { TableFindCursor } from '@/src/documents';

describe('unit.documents.tables.cursor', ({ collection }) => {
  it('should initialize an uninitialized Cursor', () => {
    const cursor = new TableFindCursor(collection, null!, [{}, false]);
    assert.ok(cursor);
  ...
  it('should fail to set projection after mapping', () => {
    const cursor1 = new TableFindCursor(collection, null!, [{}, false]);
