...
describe(`AstraTsClient - ${testClientName} Connection - collections.collection`, async () => {
  ...
  describe("insertMany tests", () => {
    ...
    it("should not insert more than allowed number of documents in one insertMany call", async () => {
      ...
      assert.strictEqual(
        error.errors[0].message,
        "Request invalid, the field postCommand.command.documents not valid: amount of documents to insert is over the max limit (21 vs 20).",
      );
    });
    it("should error out when docs list is empty in insertMany", async () => {
      ...
      assert.strictEqual(
        error.errors[0].message,
        "Request invalid, the field postCommand.command.documents not valid: must not be empty.",
      );
    });
    it("should insertMany documents ordered", async () => {
      const docList: { _id?: string; username: string }[] = Array.from(
        { length: 20 },
        () => ({ username: "id" }),
      );
      docList.forEach((doc, index) => {
    ...
    it("should error out when one of the docs in insertMany is invalid with ordered true", async () => {
      ...
      assert.strictEqual(
        error.errors[0].message,
        "Failed to insert document with _id 'docml10': Document already exists with the given _id",
      );
  ...
  describe("findOne, findMany & filter tests", () => {
    it("should find & findOne document", async () => {
      const insertDocResp = await collection.insertOne(
        createSampleDocWithMultiLevel(),
      );
    ...
    it("should find doc - return only selected fields (array slice)", async () => {
      ...
      const docList: Doc[] = Array.from({ length: 20 }, () => ({
        username: "id",
        address: { city: "nyc" },
      }));
      docList.forEach((doc, index) => {
      ...
      //read that back with projection
      const findDocs = await collection
        .find(
          {},
          {
            projection: {
  ...
  describe("updateOne tests", () => {
    ...
    it("should make _id an ObjectId when upserting with no _id", async () => {
      ...
      assert.ok(
        updateOneResp.upsertedId.match(/^[a-f\d]{24}$/i),
        updateOneResp.upsertedId,
      );
    });
  ...
  describe("updateMany tests", () => {
    ...
    it("should fail when moreData returned by updateMany as true", async () => {
      ...
      //insert next 20
      const docListNextSet = Array.from({ length: 20 }, () => ({
        username: "id",
        city: "nyc",
      }));
      ...
      assert.strictEqual(
        Object.keys(resNextSet.insertedIds).length,
        docListNextSet.length,
      );
      ...
      assert.strictEqual(
        error.message,
        'Command "updateMany" failed with the following error: More than 20 records found for update by the server',
      );
    ...
    it("should increment decimal when $inc is used", async () => {
      ...
      allDocs.forEach((doc) => {
        ...
        if (docIdNum === 5) {
          ...
          assert.strictEqual(
            doc.count,
            parseInt(doc._id.substring(2)) + 0.5 + 1,
          );
        }
      });
    });
    ...
    it("should rename a sub doc field when $rename is used in update and updateMany", async () => {
      ...
      const updateOneResp = await collection.updateOne(
        { _id: "id5" },
        { $rename: { "address.zip": "address.pincode" } },
      );
      assert.strictEqual(updateOneResp.matchedCount, 1);
      assert.strictEqual(updateOneResp.modifiedCount, 1);
      assert.strictEqual(updateOneResp.acknowledged, true);
    ...
    it("should set fields under $setOnInsert when upsert is true in updateOne", async () => {
      ...
      const updateOneResp = await collection.updateOne(
        { _id: "id5" },
        { $set: { country: "India" }, $setOnInsert: { pincode: 620020 } },
        { upsert: true },
      );
      assert.strictEqual(updateOneResp.matchedCount, 1);
      assert.strictEqual(updateOneResp.modifiedCount, 1);
      ...
      const updateOneResp1 = await collection.updateOne(
        { _id: "id21" },
        { $set: { country: "India" }, $setOnInsert: { pincode: 620020 } },
        { upsert: true },
      );
    ...
    it("should set fields under $setOnInsert when upsert is true in updateMany", async () => {
      ...
      const updateManyResp1 = await collection.updateMany(
        { _id: "id21" },
        { $set: { country: "India" }, $setOnInsert: { pincode: 620020 } },
        { upsert: true },
      );
    ...
    it("should push an element to an array when each item in $each is added using $push with $position", async () => {
      ...
      const updateOneResp = await collection.updateOne(
        { _id: "id4" },
        { $push: { tags: { $each: ["tag3", "tag4"], $position: 1 } } },
      );
      assert.strictEqual(updateOneResp.matchedCount, 1);
      assert.strictEqual(updateOneResp.modifiedCount, 1);
      assert.strictEqual(updateOneResp.acknowledged, true);
    ...
    it("should push an element to an array skipping duplicates when an item is added using $addToSet", async () => {
      ...
      const updateOneResp2 = await collection.updateOne(
        { _id: "id4" },
        { $addToSet: { tags: "tag3" } },
      );
      ...
      const updateManyResp2 = await collection.updateMany(
        {},
        { $addToSet: { tags: "tag3" } },
      );
  ...
  describe("findOneAndUpdate tests", () => {
    ...
    it("should make _id an ObjectId when upserting with no _id", async () => {
      ...
      assert.ok(
        value!._id!.toString().match(/^[a-f\d]{24}$/i),
        value!._id!.toString(),
      );
    });
  });
  ...
  describe("deleteMany tests", () => {
    ...
    it("should throw an error when deleteMany finds more than 20 records", async () => {
      ...
      assert.strictEqual(
        exception.message,
        'Command "deleteMany" failed with the following error: More records found to be deleted even after deleting 20 records',
      );
    ...
    it("should findOneAndUpdate without any updates to apply", async () => {
      await collection.deleteMany({});
      await collection.insertMany([{ username: "a" }]);
    ...
    it("should deleteOne with sort", async () => {
      ...
      await collection.deleteOne({}, { sort: { username: 1 } });
  ...
  describe("countDocuments tests", () => {
    ...
    it("should return count of documents for more than default page size limit", async () => {
      ...
      assert.strictEqual(
        await collection.countDocuments({ city: "trichy" }),
        20,
      );
      assert.strictEqual(
        await collection.countDocuments({ city: "chennai" }),
        0,
      );
