import { BlockingOptions, FullDatabaseInfo } from '@/src/devops/types';
...
export class AstraDbAdmin {
  ...
  async createNamespace(namespace: string, options?: BlockingOptions): Promise<void> {
  ...
  async dropNamespace(namespace: string, options?: BlockingOptions): Promise<void> {
    await this._httpClient.request({
  ...
  async drop(options?: BlockingOptions): Promise<void> {
    await this._httpClient.request({
      method: HTTP_METHODS.Post,
