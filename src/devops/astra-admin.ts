import { BlockingOptions, DatabaseConfig, FullDatabaseInfo, ListDatabasesOptions } from '@/src/devops/types';
...
export class AstraAdmin {
  ...
  public async createDatabase(config: DatabaseConfig, options?: BlockingOptions): Promise<AstraDbAdmin> {
    const resp = await this._httpClient.request({
  ...
  async dropDatabase(_db: Db | string, options?: BlockingOptions): Promise<void> {
